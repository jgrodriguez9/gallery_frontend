...
import { TEMATICA_GET, TEMATICA_SAVE } from '../service/Route';
...
export default function TematicaForm({auth}){
    ...
    useEffect(()=>{
        if(query.get('id')){
            setLoading(true)
            Get({url: `${TEMATICA_GET}/${query.get('id')}`, access_token: auth.data.access_token})
            .then(response=>{
                const obj = {
        ...
        <div>
            {
                ...
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting, setFieldValue, resetForm }) => {                          
                        Post({url: TEMATICA_SAVE, data: values, access_token: auth.data.access_token, header:true})
                        .then(response=>{
                            setSubmitting(false)                            
                            toast.success(MSG_SUCCESS, {autoClose: 3000})
                            history.push('/tematica')
                        ...
                        <Card.Body>
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <Link to="/tematica" className="btn btn-secondary btn-sm ml-2">Cancelar</Link>
                                </Col>
                            </Row>                                          
                        </Card.Body>
                    </Card>
